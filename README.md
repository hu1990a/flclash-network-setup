...
面向小白的一站式 FlClash 网络配置 Skill。它会从只读摸底开始，引导用户完成订阅配置备份与 DNS 优化、本机代理环境变量、CLI 时区、IPv6、TUN、手机热点以及 `ai-ipcheck` 检测，并输出脱敏验收报告。
...
- 可选安装日常自动守卫：正常静默，连续确认异常后用系统通知给出推荐操作和简要原因，并支持暂停、恢复、立即检查和卸载。
